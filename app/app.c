...
void APP_StartListening(FUNCTION_Type_t function)
{
	...
	if (gVoiceWriteIndex == 0)
		// AM/FM RX mode will be set when the voice has finished
#endif
	{
...
{
	const uint32_t Frequency = FREQUENCY_RoundToStep(pInfo->freq_config_RX.Frequency + (direction * pInfo->StepFrequency), pInfo->StepFrequency);
	...
	if (Frequency < lower) {
		return FREQUENCY_RoundToStep(upper - pInfo->StepFrequency, pInfo->StepFrequency);
	}
	...
	if (gCurrentFunction == FUNCTION_TRANSMIT) {
...
#endif
		{
			...
			if (!bKeyPressed || bKeyHeld) {
				...
				if (gEeprom.PLAY_SIDE_TONE) {	// user will here the DTMF tones in speaker
					AUDIO_AudioPathOn();
					gEnableSpeaker = true;
				}
				...
				if (Code == 0xFE) {
					BK4819_TransmitTone(gEeprom.PLAY_SIDE_TONE, 1750);
				}
#ifdef ENABLE_DTMF
				else {
					BK4819_PlayDTMFEx(gEeprom.PLAY_SIDE_TONE, Code);
				}
#endif
			}
		}
